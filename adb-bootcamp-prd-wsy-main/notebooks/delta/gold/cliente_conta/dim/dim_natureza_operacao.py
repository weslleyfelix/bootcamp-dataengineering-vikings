...
dim_ou_fato = "dim"
nome_tabela_destino = 'dim_natureza_operacao'
...
try:
    v_nr_linhas = spark.table(camada_escrita+"."+nome_tabela_destino).agg(f.count('sk_natureza')).collect()[0][0]

    if v_nr_linhas > 0:
        maior_sk = spark.read.table(camada_escrita+"."+nome_tabela_destino).select('sk_natureza').agg({'sk_natureza':'max'}).collect()[0][0]
...
try:
    df_registros_novos = (
        ...
        .withColumn('sk_natureza',f.row_number().over(Window.partitionBy().orderBy('id_natureza')) + maior_sk)
        .alias('stg')
        ...
        .join(spark.table(camada_escrita+"."+nome_tabela_destino).alias('dim'),
                (f.col('stg.id_natureza')==f.col('dim.id_natureza')),"leftouter")  

        #o filtro abaixo só retorna valores caso ele não exista no destino         
        .where(f.col('dim.id_natureza').isNull())
        .select(
            'stg.sk_natureza',
            'stg.id_natureza',
            'stg.ds_natureza_operacao'
        )
