...
dim_ou_fato = "dim"
nome_tabela_destino = 'dim_gender'
...
df = (spark.table(camada_leitura+".users_app")
           .select(
                   f.col('gender').alias('genderUser')
           ) 
           .distinct()
 
           #surrogate key
           .withColumn('skGender', f.row_number().over(Window.partitionBy().orderBy('genderUser')) + 0)
           .select('skGender','genderUser',f.expr('date_format(current_date(),"yyyy-MM-dd")').alias('DT_ATUALIZACAO'))
     #
)
